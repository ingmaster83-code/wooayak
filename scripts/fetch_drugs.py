#!/usr/bin/env python3
"""
fetch_drugs.py - 의약품 API 전체 데이터 수집 → _rawdata/drugs.json 생성
...
def main():
    parser = argparse.ArgumentParser(description="의약품 API 전체 수집")
    ...
    if OUT_FILE.exists():
        ...
        if len(drugs) < len(existing) * 0.5:
            raise SystemExit(
                f"수집 건수({len(drugs)}건)가 기존 데이터({len(existing)}건)의 절반 미만입니다. "
